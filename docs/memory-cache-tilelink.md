...
B/E 仍在 SoC-level device xbar 边界 tie off。Core-local `TLSystemXbar` 已能在 cache/tracker 与下游总线之间转发 B/E。L1 cache 已能响应单 beat Probe，但当前还没有实现 manager-side directory 或 SoC-level coherence hub，因此不能作为完整 cache-coherent fabric 使用。
