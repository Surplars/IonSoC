...
I/D cache set 数由 `SoCFeatures.iCacheSets/dCacheSets` 配置，默认 256 sets，即 2 KiB direct-mapped cache。当前 perf 结果显示 I-fetch stall 中有大量 LSU overlap，短热循环也远小于默认容量，因此不应先通过扩大 I-cache 容量解决；优先方向是 instruction queue、fetch/decode 解耦和更宽 line/refill。

`FrontendQueue` 是 IF/ID 之间的小型 flushable FIFO，默认由 `SoCFeatures.frontendQueueEntries = 2` 打开。LSU 或 load-use stall 发生时，只要队列未满，PC/IF 可以继续向前取顺序或预测路径上的指令；branch redirect、trap、`fence.i` 和 debug I-cache maintenance 会清空队列。该队列只缓存已展开的 32-bit 指令、PC、压缩长度和预测元数据，不改变 I-cache 本身，也不在 cache response 到 PC stall 之间引入组合捷径。
